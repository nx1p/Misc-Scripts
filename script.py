...
# and sort them by the number of the file in the directory. Ignoring directories without files.
# 
# For example X directory could have a file in it labeled 4 and a Y directory could have a file in it labeled 3
# So Y will go before X # 
# So by doing this the script will put together what I hope is the flag for the challange.
#
...
class DIRDict:
    def __runFind(self):
      return subprocess.run("find .secret -type f | sed 's!.secret/!!'", shell=True, check=True, universal_newlines=True, stdout=subprocess.PIPE).stdout
